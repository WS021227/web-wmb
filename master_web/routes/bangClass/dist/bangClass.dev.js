...
var tools = require('../../common/util.js');
...
router.index = function (req, res) {
  ...
  async.waterfall([function (callback) {
    ...
    tools.getMasterApiQuery('/tutorial/randomness', {
      ...
      var list = result.state == 0 ? result.data.list || [] : [];
...
router.tutorial_list = function (req, res) {
  var dataarry = {},
      ...
      size = 15;
  dataarry.dc_active = 'class', data['size'] = size;
  async.waterfall([function (callback) {
    ...
    var has_next = dataarry.pop('has_next'),
        result = {
      results: dataarry
    };
...
router.category_index = function (req, res) {
  ...
  dataarry.dc_active = 'class', async.waterfall([function (callback) {
    /*专题列表*/
    tools.getMasterApiQuery('/tutorial/category/item', {
      has_child: 1,
      route: req.params.route
...
router.detail = function (req, res) {
  var id = req.params.id,
      dataarry = {};
  dataarry.dc_active = 'class', category = '';
  async.waterfall([function (callback) {
